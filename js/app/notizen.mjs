...
const notizen = {
  ...
  oeffnen () {
    ...
    feld.innerHTML = dd.file.no; // dd.file.no kann leer sein
    setTimeout(function () {
  ...
  speichern () {
    ...
    dd.file.no = vorhanden.feld_value;
  ...
  tools (a) {
    a.addEventListener("click", async function (evt) {
      ...
      if (funktion[1] === "mark") {
        await notizen.toolsMark({ cl: "user" });
  ...
  filterleiste () {
    if (!dd.file.no || !optionen.data.einstellungen["notizen-filterleiste"]) {
      return;
    }
    ...
    document.getElementById("filter-notizen-content").innerHTML = dd.file.no;
  },
