...
const drucken = {
  ...
  tastatur () {
    ...
    if (oben === "drucken") {
      drucken.start();
    } else if (shared.hauptfunktion === "geruest") {
  ...
  init (id, gn) {
    ...
    if (!drucken.kartenIds.length) { // keine Karteikarten, die gedruckt werden können
      setTimeout(() => overlayApp.schliessen(fenster), 0); // ohne Timeout wird es nicht ausgeblendet
      dialog.oeffnen({
        typ: "alert",
  ...
  initBedeutungen (gn) {
    ...
    if (!qu.gr[gn].bd.length) {
      setTimeout(() => overlayApp.schliessen(document.getElementById("drucken")), 0); // ohne Timeout wird es nicht ausgeblendet
      dialog.oeffnen({
        typ: "alert",
