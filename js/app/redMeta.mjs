...
const redMeta = {
  ...
  oeffnen () {
    ...
    // Notizen-Feld füllen
    document.getElementById("red-meta-notizen").value = dd.file.rd.no;
  ...
  tags () {
    ...
    for (const tag of tags) {
      ...
      if (tags) { // Tags vorhanden
        for (const i of dd.file.rd[keys[tag]]) {
      ...
      if (del.length) {
        for (const i of del) {
          const idx = dd.file.rd[keys[tag]].findIndex(entry => entry.id === i);
          dd.file.rd[keys[tag]].splice(idx, 1);
        }
  ...
  bearbAuflisten () {
    ...
    for (let i = 0, len = dd.file.rd.be.length; i < len; i++) {
      const p = document.createElement("p");
      cont.appendChild(p);
      // Lösch-Link
      ...
      a.dataset.bearb = dd.file.rd.be[i];
      redMeta.bearbEntfernen(a);
      // BearbeiterIn
      p.appendChild(document.createTextNode(dd.file.rd.be[i]));
    }
  },
  ...
  bearbErgaenzen () {
    const be = document.getElementById("red-meta-be");
    const va = shared.textTrim(be.value, true);
