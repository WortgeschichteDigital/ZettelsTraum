...
export { belegeBuchung as default };
...
const belegeBuchung = {
  ...
  async read () {
    ...
    this.data.file = {};
    for (const [ id, card ] of Object.entries(dd.file.ka)) {
  ...
  results () {
    ...
    function printID ({ cont, id }) {
      ...
      a.addEventListener("click", function (evt) {
        evt.preventDefault();
        bridge.ipc.invoke("cb", "writeText", this.textContent);
    ...
    function toggleBooking (a) {
      a.addEventListener("click", function (evt) {
        evt.preventDefault();
        const card = dd.file.ka[this.dataset.id];
        if (!card) {
          // card not found
          dialog.oeffnen({
            typ: "alert",
