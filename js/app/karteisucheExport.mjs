...
export { karteisucheExport as default };
...
.redaktion-status-4 {
  ...
  async exportierenAuto (vars) {
    ...
    if (!opt[vars.vorlage]) {
      bridge.ipc.invoke("cli-message", `Fehler: Vorlage ${vars.vorlage} nicht gefunden`);
      return false;
    }
    ...
    // Message
    bridge.ipc.invoke("cli-message", `Exportiere Karteiliste "${titel}" nach ${vars.ziel}`);
    ...
    // Daten speichern
    const write = await bridge.ipc.invoke("file-write", vars.ziel, content);
  ...
  async speichern (content, format) {
    const opt = {
      ...
      defaultPath: await bridge.ipc.invoke("path-join", [ dd.app.documents, `${format.content}.${format.ext}` ]),
      filters: [
        {
    ...
    // Kartei speichern
    const write = await bridge.ipc.invoke("file-write", result.filePath, content);
    if (write.message) {
      dialog.oeffnen({
        typ: "alert",
