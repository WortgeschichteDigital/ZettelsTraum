...
const redWi = {
  ...
  dropdownVerweistexte () {
    const lemmata = shared.lemmaliste();
    const [ gn ] = document.getElementById("red-wi-gn").value.match(/[0-9]+/);
    const set = new Set();
    const felder = [ "sy", "bl" ]; // Synonmye und Wortbildungen
    for (const id of Object.keys(dd.file.ka)) {
      for (const feld of felder) {
        const daten = dd.file.ka[id][feld];
        if (!daten) {
          continue;
        }
        for (const i of daten.split("\n")) { // Mehrzeiligkeit möglich
          ...
          if (!lemmata.has(wort) &&
              !dd.file.rd.wi.some(i => i.gn === gn && i.tx === wort)) {
            set.add(wort);
          }
        }
      }
    }
    for (const wort of Object.keys(dd.file.fv)) { // Formvarianten
      if (!lemmata.has(wort) &&
          !dd.file.rd.wi.some(i => i.gn === gn && i.tx === wort)) {
        set.add(wort);
      }
    }
    return [ ...set ].sort(shared.sortAlpha);
  },
  ...
  formInit () {
    ...
    vt.value = shared.redWiDropdown.vt[0];
    const lt = document.getElementById("red-wi-lt");
    lt.value = shared.redWiDropdown.lt[0];
    redWi.formToggle();
    redWi.formReset();
    redWi.contMake();
  },
  ...
  async formEval () {
    ...
    const inputs = document.querySelectorAll(redWi.inputs);
    const txVal = shared.textTrim(inputs[0].value, true);
    if (!txVal) {
      dialog.oeffnen({
        typ: "alert",
    ...
    if (!text.classList.contains("aus")) {
      ...
      let idVal = shared.textTrim(id.value, true);
      if (!idVal) {
        dialog.oeffnen({
          typ: "alert",
      ...
      let zlVal = shared.textTrim(zl.value, true);
      if (!zlVal) {
        dialog.oeffnen({
          typ: "alert",
      ...
      const ulVal = shared.textTrim(ul.value, true);
      if (!ulVal) {
        dialog.oeffnen({
          typ: "alert",
      ...
      ds.xl += "  <Verweisziel/>\n";
      ds.xl += "  <Fundstelle>\n";
      ds.xl += `    <Fundort>${sharedXml.fundort({ url: ulVal })}</Fundort>\n`;
      ds.xl += `    <URL>${ulVal}</URL>\n`;
    ...
    function checkSemantik (lt) {
      ...
      const seVal = shared.textTrim(se.value, true);
      if (seVal && !shared.redWiDropdown.se.includes(seVal)) {
        dialog.oeffnen({
          typ: "alert",
  ...
  formSpeichern ({ ds }) {
    ...
    const idx = dd.file.rd.wi.findIndex(i => i.gn === ds.gn && i.vt === ds.vt && i.tx === ds.tx);
    if (idx > -1) {
      dd.file.rd.wi[idx] = ds;
    } else {
      dd.file.rd.wi.push(ds);
    }
    // Datensätze sortieren
    dd.file.rd.wi.sort(shared.sortWi);
  ...
  contMake () {
    ...
    if (!dd.file.rd.wi.length ||
        !dd.file.rd.wi.some(i => i.gn === gn)) {
      if (dd.file.rd.wi.length) {
        redWi.kopierenGn = gn;
  ...
  contBearbeiten ({ p }) {
    p.addEventListener("click", function () {
      ...
      const ds = dd.file.rd.wi.find(i => i.gn === gn && i.vt === vt && i.tx === tx);
  ...
  contLoeschen ({ a }) {
    a.addEventListener("click", function (evt) {
      ...
      dialog.oeffnen({
        ...
        callback: () => {
          if (dialog.antwort) {
            const idx = dd.file.rd.wi.findIndex(i => i.gn === gn && i.vt === vt && i.tx === tx);
            dd.file.rd.wi.splice(idx, 1);
            kartei.karteiGeaendert(true);
            redWi.contMake();
          }
        },
  ...
  contXml ({ a }) {
    a.addEventListener("click", function (evt) {
      ...
      const xmlDatensatz = {
        key: "wi-single",
        gn,
        ds: dd.file.rd.wi.find(i => i.gn === gn && i.vt === vt && i.tx === tx),
      };
  ...
  kopieren () {
    const [ gn ] = document.getElementById("red-wi-copy-gn").value.match(/[0-9]+/);
    // Datensätze kopieren
    const wi = [];
    for (const i of dd.file.rd.wi) {
      if (i.gn === gn) {
    ...
    dd.file.rd.wi = dd.file.rd.wi.concat(wi);
    // Datensätze sortieren
    dd.file.rd.wi.sort(shared.sortWi);
    // Änderungsmarkierung setzen
    kartei.karteiGeaendert(true);
  ...
  xml ({ icon }) {
    icon.addEventListener("click", evt => {
      ...
      const xmlDatensatz = {
        key: "wi",
        gn,
        ds: dd.file.rd.wi.filter(i => i.gn === gn),
      };
  ...
  clipboard ({ icon }) {
    icon.addEventListener("click", evt => {
      ...
      const [ gn ] = document.getElementById("red-wi-gn").value.match(/[0-9]+/);
      const ds = dd.file.rd.wi.filter(i => i.gn === gn);
      if (!ds.length) {
        dialog.oeffnen({
          typ: "alert",
          text: "Sie haben noch keine Wortinformationen eingegeben.",
      ...
      const cats = {};
      for (const i of shared.redWiDropdown.vt) {
        cats[i] = [];
      }
