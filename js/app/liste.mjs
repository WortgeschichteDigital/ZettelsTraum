...
const liste = {
  ...
  aufbauen (filter_init) {
    ...
    if (optionen.data.belegliste.sort_typ === "da") {
      const start = liste.zeitschnittErmitteln(dd.file.ka[belege[0]].da).jahrzehnt;
      const ende = liste.zeitschnittErmitteln(dd.file.ka[belege[belege.length - 1]].da).jahrzehnt;
      let jahrzehnt = start;
      let beleg_akt = 0;
      while (!(optionen.data.belegliste.sort_aufwaerts && jahrzehnt > ende ||
        ...
        while (beleg_akt <= belege.length - 1) { // Obacht!
          ...
          const zeitschnitt_akt = liste.zeitschnittErmitteln(dd.file.ka[id].da);
    ...
    for (let i = 0, len = belege.length; i < len; i++) {
      ...
      if (optionen.data.einstellungen["belegliste-referenz"]) {
        ...
        if (dd.file.ka[id].tg.includes("unzutreffend")) {
          span.classList.add("liste-unzutreffend");
        }
        span.textContent = xml.belegId({ data: dd.file.ka[id], id });
      ...
      if (optionen.data.einstellungen["belegliste-datum-iso"]) {
        const datum = shared.datumGet({
          datum: dd.file.ka[id].da,
          erstesDatum: true,
        });
      ...
      if (zeitschnitt_akt.datum.replace(/\u00A0/g, " ") !== dd.file.ka[id].da) {
        span.title = dd.file.ka[id].da;
      ...
      // Belegvorschau
      div.appendChild(liste.belegVorschau(dd.file.ka[id], id));
  ...
  aufbauenBasis (filter_init) {
    ...
    let belege = Object.keys(dd.file.ka);
  ...
  aufbauenDetails ({ id, folgekopf, einblenden = false }) {
    ...
    if (!filter.volltextSuche.suche && optionen.data.belegliste.detail_qu ||
      ...
      let text = dd.file.ka[id].qu;
      if (dd.file.ka[id].ul) {
        text += `\n${dd.file.ka[id].ul} (Aufrufdatum: ${helfer.datumFormat(dd.file.ka[id].ud, "einfach")})`;
      }
      liste.detailErstellen({
        cont: div,
        ds: "qu",
    ...
    if (!filter.volltextSuche.suche && optionen.data.belegliste.detail_no ||
      ...
      liste.detailErstellen({
        ...
        ds: "no",
        h: "Notizen",
        text: dd.file.ka[id].no,
        id,
      });
    }
    // Meta-Infos
    if (!filter.volltextSuche.suche && optionen.data.belegliste.detail_meta) {
      liste.metainfosErstellen(dd.file.ka[id], div, "liste-meta");
    }
    if (einblenden) {
  ...
  zeitschnitteAnpassen (scroll_bak) {
    ...
    for (let i = 0, len = zeitschnitte.length; i < len; i++) {
      const reg = new RegExp(shared.escapeRegExp(`${optionen.data.belegliste.zeitschnitte}|`));
  ...
  belegeSortieren (a, b) {
    ...
    if (/^d/.test(typ)) {
      ...
      if (typ === "da") {
        ...
        datum.push(dd.file.ka[a][typ]);
        datum.push(dd.file.ka[b][typ]);
      }
      if (datum[0] !== datum[1]) {
    ...
    if (typ === "au") {
      const autor = [ dd.file.ka[a].au, dd.file.ka[b].au ];
      for (let i = 0; i < 2; i++) {
        autor[i] = autor[i].replace(/\u00A0/g, " ");
      }
      if (autor[0] !== autor[1]) {
        const autorA = autor[0];
        autor.sort(shared.sortAlpha);
  ...
  belegErstellen (id) {
    ...
    if (optionen.data.einstellungen["belegliste-buchungsicon"]) {
      ...
      if (dd.file.ka[id].tg.includes("Buchung")) {
    ...
    // Absätze erzeugen
    const belegschnittMarkiert = /class="belegschnitt"/.test(dd.file.ka[id].bs);
    const prep = liste.belegErstellenPrepP(dd.file.ka[id].bs);
  ...
  belegAbsatzEinblenden (p) {
    p.addEventListener("click", function () {
      ...
      let text;
      if (id) {
        absaetze = liste.belegErstellenPrepP(dd.file.ka[id].bs).split("\n");
      } else {
  ...
  belegWortHervorheben (schnitt, immer, keinNurMarkieren = false) {
    ...
    for (const i of helfer.formVariRegExpRegs) {
      if (keinNurMarkieren && dd.file.fv[i.wort].ma && !dd.file.fv[i.wort].nl) {
        continue;
      }
      farbe = dd.file.fv[i.wort].fa;
      nebenlemma = dd.file.fv[i.wort].nl;
      nurMarkieren = dd.file.fv[i.wort].ma;
      let reg;
      ...
      schnitt = shared.suchtrefferBereinigen(schnitt.replace(reg, setzenMark), "wort");
    }
    return schnitt;
  ...
  wortVorhanden (text) {
    ...
    if (helfer.formVariRegExpRegs.length === 1) {
      if (dd.file.fv[helfer.formVariRegExpRegs[0].wort].ma) {
    ...
    for (let i = 0, len = alleMarks.length; i < len; i++) {
      ...
      for (let j = 0, len = helfer.formVariRegExpRegs.length; j < len; j++) {
        ...
        let reg;
        if (dd.file.fv[formVari.wort].ma) {
          // ma = Wort nur markieren, sonst nicht berücksichtigen
          continue;
        }
        if (!dd.file.fv[formVari.wort].tr) { // nicht trunkiert
  ...
  detailErstellen ({ cont, ds, h, text, id }) {
    ...
    if (ds === "bd") {
      const details = bedeutungen.aufbauenH2Details(dd.file.bd, true);
      if (details) {
        h += details;
      }
  ...
  textBd (bd) {
    ...
    for (let i = 0, len = bd.length; i < len; i++) {
      if (bd[i].gr !== dd.file.bd.gn) { // nur Bedeutungen des aktuellen Gerüsts anzeigen
        continue;
      }
  ...
  headerFilterAnzeige (scroll_bak) {
    ...
    const mod = dd.app.platform === "darwin" ? "⌘" : "Strg";
  ...
  buchen (icon) {
    icon.addEventListener("click", function (evt) {
      ...
      const belege = Object.keys(dd.file.ka);
      liste.belegeSortierenCache = {};
      belege.sort(liste.belegeSortieren);
      filter.aufbauen(belege);
      // Kartei geändert
  ...
  kopieren (icon) {
    icon.addEventListener("click", function (evt) {
      ...
      if (window.getSelection().toString() &&
        ...
        popup.belegID = id;
        popup.referenz.data = dd.file.ka[id];
  ...
  referenz (icon) {
    icon.addEventListener("click", function (evt) {
      ...
      bridge.ipc.invoke("cb", "writeText", referenz);
      shared.animation("zwischenablage");
    });
  },
