...
export { annotieren as default };

const annotieren = {
  // versucht ein <mark class="user"> im markierten Text zu erzeugen
  makeUser () {
    // Annotierung unmöglich
    if (shared.hauptfunktion === "liste" && !optionen.data.belegliste.trennung ||
        shared.hauptfunktion !== "liste" && !optionen.data.beleg.trennung) {
  ...
  modText (t) {
    t.addEventListener("click", function () {
      ...
      edit.addEventListener("keydown", function (evt) {
        ...
        if (!sharedTastatur.modifiers && /^(Enter|Escape)$/.test(evt.key)) {
          evt.stopPropagation();
  ...
  modTextSpeichern (input, key, text = "") {
    ...
    let textNeu = shared.textTrim(input.value, true);
    if (key === "Escape") {
      textNeu = text;
    }
    if (!textNeu) {
