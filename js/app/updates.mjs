...
const updates = {
  ...
  async check (auto) {
    ...
    // RSS-Feed auswerten
    const rss = sharedXml.parseXML(feedback.text);
    ...
    for (let i = 0, len = entries.length; i < len; i++) {
      ...
      if (updates.verToInt(dd.app.version) >= updates.verToInt(version[0])) { // installierte Version ist höher oder gleich
        break;
      }
    ...
    bridge.ipc.invoke("updates-save-data", releaseNotes);
  ...
  hinweis () {
    ...
    if (!optionen.data.updates.online ||
        updates.verToInt(dd.app.version) >= updates.verToInt(optionen.data.updates.online)) {
  ...
  async fensterFill () {
    ...
    } else if (updates.verToInt(dd.app.version) < updates.verToInt(optionen.data.updates.online)) {
    ...
    // installiert
    td[1].replaceChild(document.createTextNode(`v${dd.app.version}`), td[1].lastChild);
    // online
    let online = "\u00A0";
    if (optionen.data.updates.online) {
  ...
  buttons (input) {
    input.addEventListener("click", function () {
      if (/suchen$/.test(this.id)) {
        ...
        bridge.ipc.invoke("open-external", "https://github.com/WortgeschichteDigital/ZettelsTraum/releases");
      }
    });
  },
};
