...
const optionen = {
  ...
  data: {
    ...
    belegliste: {
      ...
      // (Datenfeld aus dd.file.ka oder "ref" als Stellvertreter für die Belegreferenz)
      sort_typ: "da",
  ...
  async anwendenTagsInit () {
    if (!optionen.data["tags-autoload-done"]) { // Tag-Dateien aus app/resources laden
      ...
      const abgleich = await bridge.ipc.invoke("optionen-tag-dateien-abgleich");
      if (!abgleich) {
        return;
      }
  ...
  async tagsAutoLaden () {
    ...
    const xml = await bridge.ipc.invoke("get-xml-file-list", resources);
    if (xml.message) {
      optionen.anwendenTags();
      return;
    }
  ...
  async tagsCheckLaden ({ datei, typ = "-" }) {
    // Datei einlesen
    ...
    if (result.message) {
      optionen.tagsFehlerMeldungen[typ] = `${result.name}: ${result.message}`;
      return false;
    }

    // Datei parsen
    const parsed = optionen.tagsParsen(result);
    const [ xml ] = parsed;
    if (!xml) {
      return false;
    }
    ...
    // Einlesen
    if (optionen.data.tags[typ]) {
      // Typ existiert => Änderungen ggf. übernehmen
      ...
      for (const [ id, val ] of Object.entries(tagsNeu)) {
        // neue Einträge anlegen, geänderte auffrischen
        if (!data[id] ||
      ...
      // Typ existiert noch nicht => einrichten
      update = true;
  ...
  tagsParsen (content) {
    const xml = sharedXml.parseXML(content);
  ...
  speichern () {
    bridge.ipc.invoke("optionen-speichern", optionen.data, dd.win.winId);
  },
  ...
  aendereEinstellung (ele) {
    ...
    if (/^kopf-icon-/.test(e)) { // Icons im Fensterkopf
      ...
      liste.status(false);
    } else if (e === "karteikarte-tagging" && shared.hauptfunktion === "karte") {
      beleg.tagsFill();
    }
    // Optionen speichern
    optionen.speichern();
  ...
  async sichern (input) {
    ...
    const opt = {
      ...
      filters: [
        {
          name: `${dd.app.name} Einstellungen`,
          extensions: [ "zte" ],
        },
        {
          name: "Alle Dateien",
    ...
    if (/exportieren$/.test(input.id)) { // Einstellungen exportieren
      ...
      opt.defaultPath = await bridge.ipc.invoke("path-join", [ opt.defaultPath, "Einstellungen.zte" ]);
      result = await bridge.ipc.invoke("datei-dialog", {
        open: false,
        winId: dd.win.winId,
        opt,
      });
    ...
    if (/exportieren$/.test(input.id)) {
      const opt = await bridge.ipc.invoke("io", {
        action: "write",
        path: result.filePath,
        data: JSON.stringify(optionen.data),
      });
      if (opt !== true) {
        dialog.oeffnen({
          typ: "alert",
