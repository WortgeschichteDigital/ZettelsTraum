...
const importTEI = {
  ...
  makeQu () {
    ...
    const regBand = new RegExp(shared.escapeRegExp(data.band));
    if (data.band &&
  ...
  async transformXML ({ tei, type }) {
    ...
    // prepare XSLT
    const xslt = sharedXml.parseXML(importTEI.transformXSL, "application/xml");
    const processor = new XSLTProcessor();
    processor.setParameter(null, "teiType", type.replace(/^tei-?/, ""));
    processor.importStylesheet(xslt);

    // transform <TEI>
    const xmlOri = sharedXml.parseXML(tei);
  ...
  createCompleteDoc ({ text, xmlDoc }) {
    ...
    header = shared.textTrim(header, true);
