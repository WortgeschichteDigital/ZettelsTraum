...
const filter = {
  ...
  aufbauen (belege) {
    ...
    if (!belege.length) {
      ...
      filter.zeitraumStart = liste.zeitschnittErmitteln(dd.file.ka[belege[0]].da).jahr;
      filter.zeitraumEnde = liste.zeitschnittErmitteln(dd.file.ka[belege[belege.length - 1]].da).jahr;
    ...
    const bd = dd.file.bd.gr[dd.file.bd.gn].bd;
    for (let i = 0, len = bd.length; i < len; i++) {
      const id = `bedeutungen-${dd.file.bd.gn}_${bd[i].id}`;
    ...
    for (let i = 0, len = belege.length; i < len; i++) {
      ...
      for (let i = 0, len = karte.bd.length; i < len; i++) {
        if (karte.bd[i].gr !== dd.file.bd.gn) {
          continue;
        }
        bdGefunden = true;
        const idBd = `bedeutungen-${dd.file.bd.gn}_${karte.bd[i].id}`;
    ...
    for (const [ block, val ] of Object.entries(filter.typen)) {
      ...
      for (let i = 0, len = f.length; i < len; i++) {
        ...
        if (/^bedeutungen-[0-9]/.test(f[i])) {
          ...
          const bd = dd.file.bd.gr[d.groups.gr].bd;
  ...
  statistikBd (belege) {
    ...
    for (let i = 1, len = ff.length; i < len; i++) { // ab 1 => undefined ausschließen
      ...
      const darunter = filter.kartenUnterBd([ `${dd.file.bd.gn}_${d.groups.id}` ]);
  ...
  jahrBelegeFuellen (belege) {
    ...
    for (let i = 0, len = belege.length; i < len; i++) {
      const id = belege[i];
      filter.jahrBelege[id] = parseInt(liste.zeitschnittErmitteln(dd.file.ka[id].da).jahr, 10);
    }
  },
  ...
  aufbauenFilter (f, obj) {
    ...
    if (/^lemmata-/.test(f) && dd.file.fv[obj.name]) {
      const fa = document.createElement("span");
      p.insertBefore(fa, label);
      fa.classList.add("farbe", `wortFarbe${dd.file.fv[obj.name].fa}`);
    }
  ...
  volltextSuchePrep () {
    ...
    vt = shared.textTrim(vt.replace(/[<>]+/g, ""), true); // macht nur Probleme, wenn erlaubt wird, nach Spitzklammern zu suchen
    // kein Filtertext
    if (!vt) {
    ...
    chunks.forEach(function (i) {
      ...
      let reg = shared.escapeRegExp(i.charAt(0));
      for (let j = 1, len = i.length; j < len; j++) {
        reg += "(<[^>]+>)*";
        reg += shared.escapeRegExp(i.charAt(j));
      }
      reg = shared.formVariSonderzeichen(reg);
  ...
  kartenFiltern (karten) {
    ...
    forX: for (let i = 0, len = karten.length; i < len; i++) {
      ...
      if (filter.aktiveFilter.zeitraum) {
        const jahr = parseInt(liste.zeitschnittErmitteln(dd.file.ka[id].da).jahr, 10);
      ...
      // Kartendatum
      if (filter.aktiveFilter.kartendatum) {
        const datum = new Date(dd.file.ka[id][kartendatumFeld]);
        ...
        if (arr.length) {
          ...
          if (bf === "bd") { // Bedeutungen
            forY: for (let j = 0, len = arr.length; j < len; j++) {
              ...
              if (arr[j] === "undefined") {
                for (let k = 0, len = dd.file.ka[id].bd.length; k < len; k++) {
                  if (dd.file.ka[id].bd[k].gr === dd.file.bd.gn) {
                    continue forY;
                  }
                }
                okay = true;
          ...
          } else if (!dd.file.ka[id][bf] && arr.includes("undefined")) { // undefined
            okay = true;
          } else if (dd.file.ka[id][bf]) {
            for (let j = 0, len = arr.length; j < len; j++) {
              ...
              if (filter.regCacheBaum[arr[j]]) {
                ...
                reg = new RegExp(`(^|\n)${shared.escapeRegExp(arr[j])}(:|\n|$)`);
                filter.regCacheBaum[arr[j]] = reg;
              }
              // suchen
              if (reg.test(dd.file.ka[id][bf])) {
                okay = true;
                break;
              }
            }
      ...
      for (const tag of tags_aktiv) {
        if (dd.file.ka[id].tg.includes(tag) && !filter_inklusiv ||
            !dd.file.ka[id].tg.includes(tag) && filter_inklusiv) {
          continue forX;
        }
      }
      ...
      if (filter.aktiveFilter["verschiedenes-notizen"] &&
          (dd.file.ka[id].no && !filter_inklusiv ||
          !dd.file.ka[id].no && filter_inklusiv)) {
        continue;
      }
      ...
      if (filter.aktiveFilter["verschiedenes-annotierung"] &&
          (/annotierung-wort/.test(dd.file.ka[id].bs) && regAnnotierung.test(dd.file.ka[id].bs) && !filter_inklusiv ||
          !(/annotierung-wort/.test(dd.file.ka[id].bs) && regAnnotierung.test(dd.file.ka[id].bs)) && filter_inklusiv)) {
        continue;
      }
      ...
      if (filter.aktiveFilter["verschiedenes-markierung"] &&
          (dd.file.ka[id].be && !filter_inklusiv ||
          !dd.file.ka[id].be && filter_inklusiv ||
          dd.file.ka[id].be && filter_inklusiv && be > dd.file.ka[id].be)) {
        continue;
      }
    ...
    for (let i = 0, len = arr.length; i < len; i++) {
      ...
      for (let j = 0, len = bd.length; j < len; j++) {
        if (bd[j].id === id) { // hier geht der Bedeutungszweig los => danach aufnehmen
          ...
          arrErg.push(`${dd.file.bd.gn}_${bd[j].id}`);
        }
      }
    }
  ...
  kartenFilternVolltext (id) {
    ...
    for (let i = 0, len = filter.volltextSuche.ds.length; i < len; i++) {
      ...
      let text_rein;
      if (ds === "bd") {
        text_rein = liste.textBd(dd.file.ka[id][ds]).join(" ").replace(/<.+?>/g, "");
      } else if (ds === "bs") {
        text_rein = dd.file.ka[id][ds].replace(/<.+?>/g, "");
      } else {
        text_rein = dd.file.ka[id][ds];
      }
  ...
  kartendatumInit () {
    ...
    if (dd.file.ka) {
      ...
      // beim ersten Aufruf ist dd.file.ka noch nicht gefüllt;
