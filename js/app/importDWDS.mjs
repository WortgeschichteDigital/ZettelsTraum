...
const importDWDS = {
  ...
  startImportJSON (json) {
    ...
    for (const i of json) {
      ...
      for (const s of i.ctx_) {
        ...
        satz = shared.textTrim(satz, true);
      ...
      // Beleg-XML
      let xmlStr = "<Beleg>";
      xmlStr += `<Belegtext>${sharedXml.maskieren({ text: ds.bs })}</Belegtext>`;
      xmlStr += "<Fundstelle>";
      if (i.meta_.urlDate) {
        xmlStr += `<Aufrufdatum>${i.meta_.urlDate}</Aufrufdatum>`;
      }
      if (i.meta_.author) {
        xmlStr += `<Autor>${sharedXml.maskieren({ text: i.meta_.author })}</Autor>`;
      }
      if (i.meta_.bibl) {
        xmlStr += `<Bibl>${sharedXml.maskieren({ text: i.meta_.bibl })}</Bibl>`;
      }
      if (ds.da) {
      ...
      if (i.meta_.title) {
        xmlStr += `<Titel>${sharedXml.maskieren({ text: i.meta_.title })}</Titel>`;
      }
      if (i.meta_.url) {
        xmlStr += `<URL>${sharedXml.maskieren({ text: i.meta_.url })}</URL>`;
      }
      xmlStr += "</Fundstelle></Beleg>";
      const xmlDoc = sharedXml.parseXML(xmlStr);
      const xmlDocIndent = sharedXml.indent(xmlDoc);
      ds.bi = "xml-dwds";
  ...
  korrekturen ({ typ, txt, ds, titeldaten, korpus }) {
    ...
    // QUELLE
      ...
      if (datum) {
        const reg = new RegExp(shared.escapeRegExp(datum[0]));
      ...
      ds.qu = shared.typographie(ds.qu);
    ...
    // TEXTSORTE
      ...
      for (let i = 0, len = tsSp.length; i < len; i++) {
        const tsClean = shared.textTrim(tsSp[i], true);
