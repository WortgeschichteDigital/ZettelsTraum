...
const anhaenge = {
  ...
  async scan (an) {
    ...
    async function scannen (datei) {
      ...
      // scannen
      const pathInfo = await bridge.ipc.invoke("path-info", {
        file: datei,
        box: kartei.pfad,
      });
      anhaenge.data[datei] = {
  ...
  async auflistenBelege (cont) {
    for (const [ id, val ] of Object.entries(dd.file.ka)) {
      // Anhänge vorhanden?
      if (!val.an.length) {
        continue;
      }
  ...
  async oeffnen (datei, ordner = false) {
    ...
    if (ordner) {
      ...
      const err = await bridge.ipc.invoke("open-path", pfad);
      if (err) {
        dialog.oeffnen({
          typ: "alert",
  ...
  async addFiles (dateien, cont, obj) {
    // Dateien hinzufügen
    const dirname = await bridge.ipc.invoke("path-dirname", kartei.pfad);
    const reg_pfad = new RegExp(shared.escapeRegExp(`${dirname}${dd.app.pathSep}`));
    const schon = [];
    const arr = anhaenge.getArr(obj);
  ...
  async addAuto ({ fenster }) {
    ...
    const [ ordner ] = kartei.pfad.match(reg);
    const dateienA = [];
    const ausgelesen = await (async function () {
      try {
        const files = await bridge.ipc.invoke("get-file-list", ordner);
        for (const f of files) {
          if (!/^(~\$|\.)|\.(bak|db|ztj)$/.test(f)) {
            dateienA.push(f);
          }
        }
        return true;
    ...
    if (dateien.join() === dd.file.an.join()) {
      dialog.oeffnen({
        typ: "alert",
    ...
    dd.file.an = dateien;
    await anhaenge.scan(dd.file.an);
    // Änderungsmarkierung
  ...
  geaendert (cont) {
    ...
    if (typ === "kartei") {
      ...
      anhaenge.makeIconList(dd.file.an, document.getElementById("kartei-anhaenge"));
    } else if (typ === "beleg") {
      beleg.belegGeaendert(true);
    }
  },
