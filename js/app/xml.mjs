...
const xml = {
  ...
  schnitt () {
    ...
    // <Beleg>
    let schnitt = sharedXml.parseXML("<Beleg></Beleg>");
    ...
    cont.innerHTML = shared.typographie(popup.textauswahl.xml);
    ...
    helfer.clipboardHtmlErsetzen({
      cont,
      selectors: ".klammer-technisch, .belegschnitt",
    });
    ...
    // Belegtext einhängen
    const belegtext = sharedXml.parseXML(`<Belegtext>${text}</Belegtext>`);
      ...
      if (n.nodeType === Node.ELEMENT_NODE) {
        for (const c of n.childNodes) {
          ...
          if (c.nodeType === Node.ELEMENT_NODE &&
            ...
            const nichtTaggen = !!c.parentNode?.dataset?.nichtTaggen;
        ...
        const textEsc = sharedXml.maskieren({ text: n.nodeValue });
    ...
    // <Datum>
    let da = sharedXml.datum(data.da, false, true);
    if (da) {
    ...
    if (data.ul) {
      ...
      urlTag.appendChild(document.createTextNode(sharedXml.maskieren({ text: url })));
    }
    ...
    let unstrukturiertTxt = sharedXml.maskieren({ text: shared.typographie(qu) });
    ...
    // Einzüge hinzufügen
    schnitt = sharedXml.indent(schnitt);
  ...
  mergeHervorhebungen ({ text }) {
    ...
    if (h) {
      ...
      const reg = new RegExp(shared.escapeRegExp(h[0]));
      text = text.replace(reg, ersatz);
      // ein Test, ob wohlgeformtes XML produziert wurde
      const xmlDoc = sharedXml.parseXML(`<Belegtext>${text}</Belegtext>`);
      if (!xmlDoc) {
        text = bak;
      }
    }
  ...
  schnittInXmlFenster (complete) {
    ...
    if (shared.hauptfunktion === "karte" && !dd.file.ka[popup.referenz.id]) {
      dialog.oeffnen({
        typ: "alert",
    ...
    if (shared.hauptfunktion === "karte" && !optionen.data.beleg.trennung) {
      dialog.oeffnen({
        typ: "alert",
    ...
    // Daten zusammentragen
    const xmlStr = xml.schnitt();
    const datum = sharedXml.datumFormat({ xmlStr });
  ...
  belegSchnittPrep (id, xmlPrep) {
    const karte = dd.file.ka[id] || beleg.data;
  ...
  belegId ({ data = popup.referenz.data, id = popup.referenz.id }) {
    // Autor
    let autor = shared.textTrim(data.au.replace(/ ?\(.+?\)/, ""), true);
    if (!autor) {
      autor = "n-n";
    } else {
      autor = autor.split(",")[0];
      autor = autor.replace(/[;.:'"„“”‚‘»«›‹+*!?(){}[\]<>&]/g, "");
      autor = shared.textTrim(autor, true);
      autor = autor.toLowerCase();
      autor = autor.replace(/[\s/]/g, "-");
    }
    // Jahr
    let jahr = "";
    const datum = sharedXml.datum(data.da.replace(/0000[-–]/, "")).match(/[0-9]{4}/);
    if (datum) {
      jahr = datum[0];
    }
    // ID zurückgeben
