...
const kopieren = {
  ...
  uiOff (fragen = true) {
    ...
    function off () {
      ...
      overlayApp.schliessen(document.getElementById("kopieren-liste"));
    }
  },
  ...
  add (id) {
    ...
    kopieren.uiText();
    shared.animation("liste", kopieren.belege.length);
  },
  ...
  einfuegenBasisdaten (animation) {
    bridge.ipc.invoke("kopieren-basisdaten", dd.win.winId);
    if (animation) {
  ...
  async einfuegenBasisdatenEintragen (daten) {
    ...
    await kopieren.einfuegenParseClipboard();
    // keine Belegquellen
  ...
  async einfuegenParseClipboard () {
    ...
    if (daten.winId === dd.win.winId && daten.wort === kartei.wort) {
      return;
    }
  ...
  einfuegenAusfuehren () {
    ...
    // Quelle = Fenster
    bridge.ipc.invoke("kopieren-daten", parseInt(quelle, 10), dd.win.winId);
  },
  ...
  async einfuegenEinlesen (daten, duplikat = false) {
    ...
    const bdMap = {};
    if (duplikat) {
      Object.keys(dd.file.bd.gr).forEach(function (i) {
        bdMap[i] = i;
      });
    } else {
    ...
    for (let i = 0, len = daten.length; i < len; i++) {
      ...
      dd.file.ka[id_karte] = beleg.karteErstellen();
      ...
      for (let j = 0, len = ds.length; j < len; j++) {
        ...
        if (ds[j] === "bd") { // Bedeutungen
          for (const k of daten[i].bd) {
            ...
            //   k.bd = Array (identisch mit dd.file.bd.gr[ID].bd[n].bd)
            if (!bdMap[k.gr]) {
              continue;
            }

            // Bedeutung im Gerüst suchen
            const gr = dd.file.bd.gr[bdMap[k.gr]];
            const kBd = k.bd.join("|");
            let id = 0;
            ...
            dd.file.ka[id_karte].bd.push({
              gr: bdMap[k.gr],
              id,
            });
          }
        } else if (Array.isArray(daten[i][ds[j]])) { // eindimensionale Arrays
          dd.file.ka[id_karte][ds[j]] = [ ...daten[i][ds[j]] ];
        } else { // Primitiven
          dd.file.ka[id_karte][ds[j]] = daten[i][ds[j]];
        }
      }
      // Speicherdatum ergänzen
      dd.file.ka[id_karte].dm = new Date().toISOString();
    }
    ...
    if (optionen.data.einstellungen["einfuegen-schliessen"]) {
      overlayApp.schliessen(document.getElementById("kopieren-einfuegen"));
    }

    // Feedback anzeigen
    shared.animation("einfuegen");
  ...
  basisdatenSenden () {
    const daten = {
      ...
      gerueste: Object.keys(dd.file.bd.gr),
    };
    bridge.ipc.invoke("kopieren-basisdaten-lieferung", daten);
  },
  ...
  datenSenden () {
    const daten = [];
    for (const id of kopieren.belege) {
      daten.push(kopieren.datenBeleg(dd.file.ka[id]));
    }
    bridge.ipc.invoke("kopieren-daten-lieferung", daten);
  },
  ...
  datenBeleg (quelle) {
    ...
    for (const b of quelle.bd) {
      const bed = dd.file.bd.gr[b.gr].bd.find(i => i.id === b.id);
      bd.push({
        bd: [ ...bed.bd ],
        gr: b.gr,
      });
  ...
  async exportieren () {
    ...
    for (const id of kopieren.belege) {
      daten.bl.push(kopieren.datenBeleg(dd.file.ka[id]));
    }
    ...
    const opt = {
      ...
      defaultPath: await bridge.ipc.invoke("path-join", [ dd.app.documents, `${kartei.titel}, ${kopieren.belege.length} ${num}.ztb` ]),
      filters: [
        {
          name: `${dd.app.name} Belege`,
          extensions: [ "ztb" ],
        },
        {
          name: "Alle Dateien",
    ...
    // Datei schreiben
    const ergebnis = await bridge.ipc.invoke("io", {
      action: "write",
  ...
  async importieren () {
    ...
    if (belegedatei_tmp.ty !== "ztb") {
      dialog.oeffnen({
        typ: "alert",
        text: `Beim Einlesen der Datei ist ein Fehler aufgetreten.\n<h3>Fehlermeldung</h3>\nkeine <i>${dd.app.name} Belege</i>-Datei`,
      });
      return;
    }
