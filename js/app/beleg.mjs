...
import importDereko from "./importDereko.mjs";
import importDWDS from "./importDWDS.mjs";
...
const beleg = {
  ...
  idErmitteln () {
    let id_karte = 0;
    const ids = Object.keys(dd.file.ka);
  ...
  oeffnen (id) {
    ...
    beleg.data = structuredClone(dd.file.ka[id]);
  ...
  formularBedeutungLabel () {
    const text = `Bedeutung${bedeutungen.aufbauenH2Details(dd.file.bd, true)}`;
  ...
  formularBedeutungEx (icon) {
    icon.addEventListener("click", function (evt) {
      ...
      const idx = beleg.data.bd.findIndex(i => i.gr === dd.file.bd.gn && i.id === id);
  ...
  formularBedeutungList () {
    ...
    const gr = dd.file.bd.gr[dd.file.bd.gn];
    const bd = [];
    for (let idx = 0, len = gr.bd.length; idx < len; idx++) {
      const b = gr.bd[idx];
      if (beleg.data.bd.some(i => i.gr === dd.file.bd.gn && i.id === b.id)) {
        continue;
      }
  ...
  formularBedeutungAdd (val) {
    ...
    for (let idx = 0, len = bd.length; idx < len; idx++) {
      ...
      if (bdAkt.includes(valNorm)) {
        ...
        if (beleg.data.bd.some(i => i.gr === dd.file.bd.gn && i.id === id)) {
  ...
  formularBedeutungPush (id, gr = dd.file.bd.gn) {
    beleg.data.bd.push({
      gr,
      id,
    });
  ...
  formularBedeutungSort (a, b) {
    const x = dd.file.bd.gr[a.gr].bd.findIndex(i => i.id === a.id);
    const y = dd.file.bd.gr[b.gr].bd.findIndex(i => i.id === b.id);
    return x - y;
  },
  ...
  async formularImport ({ src, fokus = true, typeData = null, autoFill = true }) {
  ...
  formularEvtFormData (input) {
    input.addEventListener("keydown", function (evt) {
      ...
      if ((!sharedTastatur.modifiers || sharedTastatur.modifiers === "Ctrl") && evt.key === "Enter") {
  ...
  formularEvtImport () {
    ...
    feld.addEventListener("focus", async function () {
  ...
  aktionSpeichern (nieSchliessen = false) {
    ...
    const dav = shared.textTrim(da.value, true);
    if (!dav) {
    ...
    if (!shared.textTrim(bs.value, true)) {
      if (!optionen.data.einstellungen["karteikarte-keine-fehlermeldung"]) {
        dialog.oeffnen({
          typ: "alert",
    ...
    const quVal = shared.textTrim(qu.value, true);
    if (!quVal) {
      if (!optionen.data.einstellungen["karteikarte-keine-fehlermeldung"]) {
        dialog.oeffnen({
          typ: "alert",
    ...
    // ggf. Objekt anlegen
    if (!dd.file.ka[beleg.id_karte]) {
      dd.file.ka[beleg.id_karte] = {};
    ...
    dd.file.ka[beleg.id_karte] = structuredClone(beleg.data);
    // Änderungsdatum speichern
    const dm = new Date().toISOString();
    beleg.data.dm = dm;
    dd.file.ka[beleg.id_karte].dm = dm;
    // Metadatenfelder füllen
    beleg.metadaten();
  ...
  toolsKopierenKlammern ({ text, html = false }) {
    ...
    return shared.textTrim(text, true);
  },
  ...
  toolsKopierenAddJahr (text, html) {
    ...
    const jahr = sharedXml.datum(popup.referenz.data.da, false, true); // könnte auch Jh. sein
  ...
  async toolsEinfuegen (link) {
    ...
    if (id === "beleg-bs") {
      ...
      text = await bridge.ipc.invoke("cb", "readText");
    }
    ...
    function ana (ele, preformatted) {
      if (ele.nodeType === 3) { // Text
        ...
        if (/^(DT|FIGCAPTION|HR|LI|TR)$/.test(ele.nodeName)) { // Zeilenumbruch
          ...
          text = shared.textTrim(text, false);
          text += "\n\n";
  ...
  toolsText (link) {
    ...
    // Aktion durchführen
    const reg_start = new RegExp(`${shared.escapeRegExp(tags[aktion].start)}$`);
    const reg_ende = new RegExp(`^${shared.escapeRegExp(tags[aktion].ende)}`);
  ...
  async toolsTextdatum (shortcut = false, autoImport = false) {
    ...
    const dateCurrent = beleg.data.da ? shared.datumGet({
      datum: beleg.data.da,
      erstesDatum: true,
    }).sortier : "";
  ...
  async toolsQuelleLaden (shortcut = false) {
    ...
    if (/^(tei|xml)/.test(beleg.data.bi)) {
      xmlDoc = sharedXml.parseXML(beleg.data.bx);
    }
  ...
  toolsKuerzen () {
    ...
    for (let i = 0, len = text.length; i < len; i++) {
      ...
      if (i && (!pbAll.length || !new RegExp(`^\\[:${shared.escapeRegExp(pbAll[0])}:\\]`).test(text[i]))) {
    ...
    const [ seiteStart ] = pb[kurzErhalten[0]];
  ...
  leseToggle (user) {
    ...
    if (beleg.leseansicht) {
      ...
      button.title = `zur Leseansicht wechseln (${sharedTastatur.shortcutsTextAktuell("Strg")} + U)`;
      tab.classList.remove("leseansicht");
    } else {
      beleg.leseansicht = true;
      button.classList.remove("beleg-opt-anzeige-letztes");
      button.title = `zur Formularansicht wechseln (${sharedTastatur.shortcutsTextAktuell("Strg")} + U)`;
    ...
    if (beleg.leseansicht) {
      springen.title = `zur nächsten Markierung springen (${sharedTastatur.shortcutsTextAktuell("Strg")} + ↓)`;
    } else {
      springen.title = `zum Wort im Belegtext springen (${sharedTastatur.shortcutsTextAktuell("Strg")} + ↓)`;
    }
    ...
    if (beleg.leseansicht) {
      ...
      document.querySelectorAll("#beleg textarea").forEach(textarea => shared.textareaGrow(textarea));
  ...
  ctrlKuerzenAnzeige () {
    ...
    if (optionen.data.beleg.kuerzen) {
      ...
      link.title = `Belegkontext anzeigen (${sharedTastatur.shortcutsTextAktuell("Strg")} + K)`;
    } else {
      link.classList.remove("aktiv");
      link.title = `Belegkontext kürzen (${sharedTastatur.shortcutsTextAktuell("Strg")} + K)`;
    }
  ...
  ctrlTrennungAnzeige () {
    ...
    if (optionen.data.beleg.trennung) {
      link.classList.add("aktiv");
      link.title = `Silbentrennung nicht anzeigen (${sharedTastatur.shortcutsTextAktuell("Strg")} + T)`;
    } else {
      link.classList.remove("aktiv");
      link.title = `Silbentrennung anzeigen (${sharedTastatur.shortcutsTextAktuell("Strg")} + T)`;
    }
    tooltip.init(link.parentNode);
  },
  ...
  ctrlZwischenablage (dt) {
    ...
    daten.typ = "ztb";
    daten.version = 3;
    daten.winId = dd.win.winId;
    daten.wort = kartei.wort;
    bridge.ipc.invoke("cb", "writeText", JSON.stringify(daten));
    shared.animation("zwischenablage");
  },
  ...
  async bedeutungenWinListe (bd, eintragen) {
    ...
    document.querySelectorAll("#liste-belege-cont .liste-kopf").forEach(kopf => {
      ...
      const idx = dd.file.ka[id].bd.findIndex(i => i.gr === bd.gr && i.id === bd.id);
      if (eintragen) {
        if (idx >= 0) {
          return;
        }
        dd.file.ka[id].bd.push({ ...bd });
        dd.file.ka[id].bd.sort(beleg.formularBedeutungSort);
      } else {
        if (idx === -1) {
          return;
        }
        dd.file.ka[id].bd.splice(idx, 1);
      }
      dd.file.ka[id].dm = new Date().toISOString();
      geaendert = true;
    });
  ...
  async metadatenReimport () {
    ...
    await bridge.ipc.invoke("cb", "writeText", bx);
  ...
  metadatenCopy (id) {
    ...
    // Daten kopieren
    bridge.ipc.invoke("cb", "writeText", beleg.data[field]);
