...
import importLit from "./importLit.mjs";
import importTEI from "./importTEI.mjs";
import importURL from "./importURL.mjs";
import importWGd from "./importWGd.mjs";
...
const importShared = {
  ...
  isKnownXML (str) {
    ...
    const xml = sharedXml.parseXML(str);
    if (!xml) {
      return null;
    }
  ...
  async fileRead () {
    ...
    pfad = await bridge.ipc.invoke("path-normalize", pfad.replace(/^file:\/\//, ""));
    ...
    async function readFile () {
      let fileCont = await bridge.ipc.invoke("file-read", {
        path: pfad,
        enc: encodings[encoding],
      });

      if (fileCont.message) {
        fehler(`${fileCont.name}: ${fileCont.message}`);
        return false;
      }
  ...
  fileDataSchonImportiert () {
    ...
    for (const beleg of importShared.fileData.dataOld) {
      if (!beleg.sha1) {
        const ds = { ...beleg.ds };
        ds.di = "";
        beleg.sha1 = crypto.createHash("sha1").update(JSON.stringify(ds)).digest("hex");
      }
    }
    ...
    for (const beleg of importShared.fileData.data) {
      const ds = { ...beleg.ds };
      ds.di = "";
      const sha1 = crypto.createHash("sha1").update(JSON.stringify(ds)).digest("hex");
  ...
  async importFromOriginalSource (ds) {
    ...
    if (/^tei-(dibilit|dibiphil|humboldt|stimmlos)$/.test(source.type) && ds.bx) {
      const xmlDoc = sharedXml.parseXML(ds.bx);
      if (xmlDoc) {
  ...
  checkQuotation (bs = beleg.data.bs, pos = false) {
    ...
    if (!wortGefunden) {
      const verianten = [];
      for (const [ k, v ] of Object.entries(dd.file.fv)) {
