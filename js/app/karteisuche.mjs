...
const karteisuche = {
  ...
  async oeffnen () {
    ...
    // Cache laden
    karteisuche.ztjCache = await bridge.ipc.invoke("ztj-cache-get");
  ...
  async suchenPrep () {
    ...
    if (inputs.length) {
      for (const i of inputs) {
        ...
        abgehakt++;
        const exists = await bridge.ipc.invoke("file-exists", i.value);
        if (exists) {
          pfade.push(i.value);
      ...
      abgehakt++;
      const [ pfad ] = optionen.data.karteisuche.pfade;
      const exists = await bridge.ipc.invoke("file-exists", pfad);
      if (exists) {
        pfade.push(pfad);
  ...
  async suchenPrepZtj (pfade) {
    const status = await bridge.ipc.invoke("ztj-cache-status-get");
    ...
    bridge.ipc.invoke("ztj-cache-status-set", true);
      ...
      setTimeout(async () => {
        ...
        if (pfade.length) {
          ...
          for (const ordner of pfade) {
            const exists = await bridge.ipc.invoke("file-exists", ordner);
            if (exists) {
              try {
                // Lesezugriff auf Basisordner? Wenn kein Zugriff => throw
                const readable = await bridge.ipc.invoke("path-readable", ordner);
                if (readable.message) {
                  throw readable;
                }
          ...
          document.querySelectorAll("#karteisuche-pfade span[data-pfad]").forEach(i => {
            ...
            if (input && input.checked ||
                !input) {
              const reg = new RegExp("^" + shared.escapeRegExp(i.dataset.pfad));
              pfade.push(reg);
            }
          });
  ...
  async suchen () {
    ...
    for (const kartei of karteisuche.ztj) {
      ...
      if (karteisuche.ztjCache[kartei.pfad] &&
        ...
        // Kartei cachen
        bridge.ipc.invoke("ztj-cache-save", {
          pfad: kartei.pfad,
          ctime: kartei.ctime,
          data: datei,
        });
      ...
      for (let i = 0, len = woerter.length; i < len; i++) {
        ...
        let er = datei.rd.er;
        if (!er) {
          // bis Dateiformat Version 12 standen die Redaktionsereignisse in dd.file.rd;
          // erst danach in dd.file.rd.er
          er = datei.rd;
        }
        for (const erg of er) {
    ...
    void new Notification(dd.app.name, notifyOpts);
  },
  ...
  suchenAbschluss () {
    ...
    bridge.ipc.invoke("ztj-cache-status-set", false);
  },
  ...
  //   redaktion (Array; Klon von dd.file.rd.er)
  ...
  async ordnerParsen (ordner, suchtiefe) {
    suchtiefe++;
    try {
      const files = await bridge.ipc.invoke("dir-read", ordner);
      for (const i of files) {
        const pfad = await bridge.ipc.invoke("path-join", [ ordner, i ]);
  ...
  async pfadPruefen (pfad, suchtiefe) {
    ...
    if (stats && !stats.exists) {
      // wahrscheinlich besteht kein Zugriff auf den Pfad
      return false;
    } else if (stats?.isDirectory && // Ordner => parsen
    ...
    if (karteisuche.filterWerte.some(i => i.typ === "Redaktion")) {
      ...
      for (const e of erstellt) {
        for (const i of e.arr) {
          ...
          const [ da ] = helfer.datumFormat(i.da, "minuten").split(", ");
  ...
  ztjAuflistenVerweise (i) {
    ...
    verweise.sort(shared.sortAlpha);
  ...
  ztjOeffnen (a) {
    a.addEventListener("click", function (evt) {
      ...
      bridge.ipc.invoke("kartei-laden", this.dataset.pfad, false);
    });
  },
  ...
  *idGenerator (id) {
  ...
  filterWerteSammeln () {
    ...
    for (const filter of document.querySelectorAll(".karteisuche-filter")) {
      ...
      if (typ === "Karteiwort") {
        ...
        obj.reg = new RegExp(shared.formVariSonderzeichen(shared.escapeRegExp(text)), "i");
        ...
        text = shared.textTrim(text.replace(/[<>]+/g, ""), true);
        if (!text) {
          karteisuche.filterIgnorieren(filter, true);
          continue;
        }
        ...
        textEr = shared.textTrim(textEr, true);
        textPr = shared.textTrim(textPr, true);
        if (!textEr && !textPr) {
          karteisuche.filterIgnorieren(filter, true);
          continue;
        }
        if (textEr) {
          obj.er = new RegExp(shared.formVariSonderzeichen(shared.escapeRegExp(textEr)), "i");
        }
        if (textPr) {
          obj.pr = new RegExp(shared.formVariSonderzeichen(shared.escapeRegExp(textPr)), "i");
        }
  ...
  filterVolltext: {
    // dd.file.wo wurde mit v26 entfernt
    datei: [ "no", "wo" ],
    // dd.file.rd.bh und dd.file.rd.nl wurden mit v26 entfernt
    redaktion: [ "bh", "nl", "no" ],
  ...
  filtern (datei) {
    let be = datei.rd.be;
    if (!be) {
      // bis Dateiformat Version 12 standen die Bearbeiterinnen in dd.file.be;
      // erst danach in dd.file.rd.be
      be = datei.be;
    }
    let er = datei.rd.er;
    if (!er) {
    ...
    document.querySelectorAll(".karteisuche-redaktion-ereignis").forEach(i => {
      redErg.add(shared.formVariSonderzeichen(shared.escapeRegExp(i.value)));
    });
