...
const redXml = {
  ...
  async oeffnen () {
    ...
    if (redXml.contentsId) {
      bridge.ipc.invoke("red-xml-fokussieren", redXml.contentsId);
      return false;
    }
    // Fenster durch Main-Prozess öffnen lassen
    redXml.contentsId = await bridge.ipc.invoke("red-xml-oeffnen", `XML: ${kartei.titel}`);
    return true;
  },
  ...
  async schliessen () {
    ...
    await bridge.ipc.invoke("red-xml-schliessen", redXml.contentsId);
    redXml.contentsId = 0;
    return true;
  },
  ...
  daten () {
    ...
    for (const [ k, v ] of Object.entries(dd.file.bd.gr)) {
