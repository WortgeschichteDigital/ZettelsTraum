...
const kopf = {
  ...
  icons () {
    ...
    const kopfLeiste = document.getElementById("kopf-icons");
    const last = kopfLeiste.querySelector(".last");
    if (last) {
      last.classList.remove("last");
    }
    const aAn = kopfLeiste.querySelectorAll("a:not(.aus)");
    if (aAn.length) {
      aAn[aAn.length - 1].classList.add("last");
    }
    // ggf. die Anhänge-Icons anzeigen
    const anhaenge = document.getElementById("kartei-anhaenge");
  ...
  // Karteiordner-Icon
  iconOrdner () {
    const iconOrdner = document.getElementById("ordner-icon");
  ...
  iconRedaktionSvg (abgeschlossen) {
    const svg = document.querySelector("#redaktion-icon svg");
  ...
  // Notizen-Icon
  iconNotizen () {
    const icon = document.getElementById("notizen-icon");
  ...
  // Lexika-Icon
  iconLexika () {
    const icon = document.getElementById("lexika-icon");
