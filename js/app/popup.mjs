...
const popup = {
  ...
  referenz: {
    data: {}, // Objekt, in dem die Karteikarte liegt, d.i. beleg.data | dd.file.ka[ID]
  ...
  oeffnen (evt) {
    ...
    if (target === "kopieren") {
      ...
      } else if (/^(karte|liste)$/.test(shared.hauptfunktion)) {
      ...
      if (oben === "stamm") {
        ...
        items.push("link", "sep", "titelBearbeiten", "titelAufnahmen", { name: "titelAufnahmeCp", sub: [ "titelAufnahmeText", "titelAufnahmeXml" ] }, { name: "titelReferenzCp", sub: [ "titelReferenzText", "titelReferenzXml" ] }, "titelSigle", "sep", "importConf", "sep", "schliessen");
        if (kartei.wort) {
      ...
      items.push("titelBearbeiten", "titelAufnahmen", { name: "titelAufnahmeCp", sub: [ "titelAufnahmeText", "titelAufnahmeXml" ] }, { name: "titelReferenzCp", sub: [ "titelReferenzText", "titelReferenzXml" ] }, "titelSigle", "sep", "importConf", "sep", "schliessen");
      if (kartei.wort) {
    ...
    for (let i = 0, len = pfad.length; i < len; i++) {
      ...
      if (pfad[i].classList) {
        if (pfad[i].classList.contains("start-datei")) {
          ...
          } else if (oben !== "stamm" && shared.hauptfunktion === "karte") {
            popup.referenz.data = beleg.data;
            popup.referenz.id = "" + beleg.id_karte;
          } else if (oben !== "stamm" && shared.hauptfunktion === "liste") {
            const id = pfad[i].closest(".liste-details").previousSibling.dataset.id;
            popup.referenz.data = dd.file.ka[id];
            popup.referenz.id = id;
          }
          ...
          popup.belegID = pfad[i].dataset.id;
          popup.referenz.data = dd.file.ka[popup.belegID];
  ...
  getTargetSelection (pfad) {
    ...
    while (ele.nodeName !== "BODY") {
      ...
      if (ele.classList.contains("liste-details")) {
        ...
        karte = dd.file.ka[id];
        popup.referenz.data = dd.file.ka[id]; // für xml.referenz();
        popup.referenz.id = id;
        break;
  ...
  textauswahlComplete (copy) {
    const bs = true;
    const karte = dd.file.ka[popup.belegID] || beleg.data;
