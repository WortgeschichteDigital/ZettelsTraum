...
const winShared = {
  ...
  ipcListener () {
    ...
    bridge.ipc.listen("xml-daten", async xmlDaten => {
    ...
    bridge.ipc.listen("xml-datensatz", xmlDatensatz => xml.empfangen({ xmlDatensatz }));
  ...
  xmlPrettyPrint () {
    ...
    for (const i of pretty) {
      i.innerHTML = sharedXml.prettyPrint({ xmlStr: i.textContent });
    }
  },
  ...
  events () {
    ...
    // Nebenfenster öffnen
    document.querySelectorAll(".link-changelog").forEach(a => helfer.oeffneChangelog(a));
    document.querySelectorAll(".link-dokumentation, .link-handbuch").forEach(a => helfer.oeffne(a));
    document.querySelectorAll(".link-fehlerlog").forEach(a => helfer.oeffneFehlerlog(a));
    ...
    // externe Links
    document.querySelectorAll('a[href^="http"], a[href^="mailto"]').forEach(a => shared.externeLinks(a));
  ...
  eventsXml () {
    ...
    document.addEventListener("copy", () => {
      setTimeout(async () => {
        ...
        let text = await bridge.ipc.invoke("cb", "readText");
        text = text.replace(/␣/g, "\u00A0").replace(/[.]{3}/g, "…");
        await bridge.ipc.invoke("cb", "clear");
