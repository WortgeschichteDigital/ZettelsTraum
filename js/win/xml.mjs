...
const xml = {
  ...
  dropdownLesarten () {
    ...
    const xmlDoc = sharedXml.parseXML(xml.data.xl.bg[xml.bgAkt].xl);
    if (!xmlDoc) {
      data.err = true;
      return data;
    }
  ...
  *counterGenerator (n) {
  ...
  async reset () {
    ...
    // Daten zurücksetzen
    xml.data.xl = sharedXml.redXmlData();
    xml.speichern();
  ...
  mdRevisionAdd () {
    ...
    const noVal = shared.typographie(shared.textTrim(no.value, true));
    for (let i = 0, len = auVal.length; i < len; i++) {
      auVal[i] = shared.textTrim(auVal[i], true);
    }
    // Überprüfungen
    if (!auVal.length) {
      dialog.oeffnen({
  ...
  mdChange ({ input }) {
    input.addEventListener("change", function () {
      ...
      let val = shared.textTrim(this.value, true);
      // Validierung
      if (key === "id") {
        let id = sharedXml.normId({ id: val });
        if (id) {
          if (!/^WGd-/.test(id)) {
            id = `WGd-${id}`;
          }
  ...
  lemmaAdd () {
    ...
    for (let i = 0, len = leVal.length; i < len; i++) {
      leVal[i] = shared.textTrim(leVal[i], true);
    }
    // Überprüfungen
    if (!leVal.length) {
      dialog.oeffnen({
    ...
    if (/^(bl|lt)$/.test(xmlDatensatz.key)) {
      ...
      if (!xml.data.xl.wi?.[xml.bgAktGn]?.length) {
        ...
        if (slot > -1) { // Datensatz ersetzen
          ...
          xml.data.xl.wi[xml.bgAktGn].sort(shared.sortWi);
          // Kopf ersetzen
          const slot = xml.data.xl.wi[xml.bgAktGn].findIndex(i => i.vt === xmlDatensatz.ds.vt && i.tx === xmlDatensatz.ds.tx);
          const kopf = xml.elementKopf({
            key: "wi",
  ...
  async belegEinfuegen () {
    ...
    // Datensatz erstellen
    const datum = sharedXml.datumFormat({ xmlStr: cb });
    const xmlDatensatz = {
      key: "bl",
      ds: {
        da: datum.anzeige,
  ...
  async bgNachweisAdd () {
    ...
    if (tyVal === "Literatur") {
      ...
      const stVal = shared.textTrim(document.getElementById("nw-lit-st").value, true);
      if (!stVal) {
      ...
      const txVal = shared.textTrim(tx.value, true);
      if (!txVal) {
        dialog.oeffnen({
          typ: "alert",
      ...
      const fundort = sharedXml.fundort({ url: ulVal });
  ...
  abschnittSetId ({ key, slot, slotBlock, loeschen = false }) {
    ...
    if (xl && !loeschen) { // nach dem Löschen einer Überschrift übergehen
      ...
      id = id.replace(/<.+?>/g, "");
      id = shared.textTrim(id, true);
      id = sharedXml.normId({ id });
    }
  ...
  textblockAbbXml ({ form }) {
    ...
    bu.value = shared.typographie(shared.textTrim(bu.value, true));
    ...
    at.value = shared.typographie(shared.textTrim(at.value, true));
    xl += `\t<Bildinhalt>${mask(at.value)}</Bildinhalt>\n`;
    xl += "\t<Fundstelle>\n";
    const qu = form.querySelector('[id$="quelle"]');
    qu.value = shared.typographie(shared.textTrim(qu.value, true));
  ...
  abtxChange ({ ele }) {
    ele.addEventListener("change", function () {
      ...
      if (slotBlock !== null &&
        ...
        if (feld === "id") {
          // ID aufbereiten
          val = sharedXml.normId({ id: val, input: this });
        }
  ...
  preview ({ xmlStr, key, slot, slotBlock = null, after = null, textblockCont = null, animation = true, editable = false }) {
    ...
    let xmlDoc = sharedXml.parseXML(xmlStr);
    if (xmlDoc) {
    ...
    if (preCont && preCont.classList.contains("pre-cont")) {
      preCont.firstChild.innerHTML = sharedXml.prettyPrint({ xmlStr, xmlErr });
      return;
    }
    ...
    pre.innerHTML = sharedXml.prettyPrint({ xmlStr, xmlErr });
    if (after) {
      ...
      if (this.value === "Speichern") {
        ...
        if (key === "bl") {
          ...
          if (da && da[1] !== xml.data.xl.bl[slot].da) {
            ...
            xml.data.xl.bl[slot].ds = sharedXml.datumFormat({ xmlStr }).sortier;
  ...
  editAutoTagger ({ str, blockzitat }) {
    ...
    str = str.replace(/\[([^\]]+?)\]\(\s*(https?:\/\/[^\s]+?)\s*\)(?:\(\s*([0-9]{1,2})\.\s*([0-9]{1,2})\.\s*([0-9]{4})\s*\))?/g, (...args) => {
      ...
      const fundort = sharedXml.fundort({ url: args[2] });
    ...
    // (Aufhübschung von shared.typographie() rückgängig machen)
    ...
    // <Abkuerzung>
    if (blockzitat) {
      str = str.replace(/<Autorenzusatz>(.+?)<\/Autorenzusatz>/gs, (m, p1) => `<Autorenzusatz>${sharedXml.abbrTagger({ text: p1 })}</Autorenzusatz>`);
    } else {
      str = sharedXml.abbrTagger({ text: str });
      str = str.replace(/<Zitat>(.+?)<\/Zitat>/g, (m, p1) => {
        let zitat = p1.replace(/<Abkuerzung Expansion=".+?">(.+?)<\/Abkuerzung>/g, (m, p1) => p1);
        zitat = zitat.replace(/<Autorenzusatz>(.+?)<\/Autorenzusatz>/g, (m, p1) => `<Autorenzusatz>${sharedXml.abbrTagger({ text: p1 })}</Autorenzusatz>`);
  ...
  exportieren () {
    ...
    function indentXml (str, level) {
      let xmlDoc = sharedXml.parseXML(str);
      xmlDoc = sharedXml.indent(xmlDoc);
  ...
  async exportierenSpeichern ({ xmlStr }) {
    ...
    const opt = {
      title: "XML speichern",
      defaultPath: await bridge.ipc.invoke("path-join", [ dd.app.documents, `${wort}.xml` ]),
      filters: [
        {
          name: "XML-Dateien",
          extensions: [ "xml" ],
    ...
    if (xml.data.letzter_pfad) {
      opt.defaultPath = await bridge.ipc.invoke("path-join", [ xml.data.letzter_pfad, `${wort}.xml` ]);
    }
    ...
    // Kartei speichern
    const write = await bridge.ipc.invoke("file-write", result.filePath, xmlStr);
    if (write.message) {
      dialog.oeffnen({
        typ: "alert",
        text: `Beim Speichern der XML-Datei ist ein Fehler aufgetreten.\n<h3>Fehlermeldung</h3>\n<p class="force-wrap">${write.message}</p>`,
      });
    }

    // Pfad speichern
    const reg = new RegExp(`^.+\\${dd.app.pathSep}`);
    const [ pfad ] = result.filePath.match(reg);
  ...
  async importieren () {
    ...
    // Datei einlesen
    const content = await bridge.ipc.invoke("file-read", {
    ...
    const xmlDoc = sharedXml.parseXML(content);
    if (!xmlDoc) {
      dialog.oeffnen({
        typ: "alert",
