...
const hilfe = {
  ...
  async naviSprungAusfuehren (id) {
    ...
    if (dd.win.typ === "dokumentation" || dd.win.typ === "handbuch") {
  ...
  suche (enter) {
    ...
    const val = shared.textTrim(feld.value, true);
    ...
    if (/\s/.test(val)) { // nur, wenn es mehrere Wörter gibt
      e.regPhrase = new RegExp(shared.escapeRegExp(val), "gi");
    }
    // reguläre Ausdrücke
    const val_sp = val.split(/\s/);
    val_sp.sort(shared.sortLengthAlpha);
    for (let i = 0, len = val_sp.length; i < len; i++) {
      const reg = new RegExp(shared.escapeRegExp(val_sp[i]), "gi");
      e.reg.push(reg);
    }
    // Suche durchführen
    ...
    function durchsuchen (sektion, knoten) {
      ...
      const text = shared.textTrim(knoten.innerText, true);
      let gewicht = 0;
  ...
  sucheDrucken (start) {
    ...
    for (let i = start, len = e.treffer.length; i < len; i++) {
      ...
      span.innerHTML = shared.suchtrefferBereinigen(text);
