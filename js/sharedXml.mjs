...
const sharedXml = {
  ...
  //     (das XML-Snippet)
  indent (xml) {
    const xslt = sharedXml.parseXML(sharedXml.indentXsl, "application/xml");
  ...
  prettyPrint ({ xmlStr, xmlErr = null }) {
    ...
    if (xmlErr) {
      ...
      xmlStr = sharedXml.maskieren({ text: xmlStr });
  ...
  abbrTagger ({ text, lit = false }) {
    let abbr = sharedXml.abbr;
    let notBefore = '(?<!(<Abkuerzung Expansion=".+?">|\\p{Letter}|\u00A0))';
    if (lit) {
      abbr = sharedXml.abbrLit;
